...
class NectarPostGrid {
  ...
  public function nectar_get_post_grid_segment() {
    ...
    $attributes['content_next_to_image_excerpt_pos'] = sanitize_text_field($_POST['settings']['content_next_to_image_excerpt_pos']);
    ...
    $attributes['remove_links']                    = sanitize_text_field($_POST['settings']['remove_links']);
    ...
    $attributes['vertical_list_read_more_style']   = sanitize_text_field($_POST['settings']['vertical_list_read_more_style']);
  ...
  public static function get_attributes() {
    return array(
        ...
        'grid_item_height_subtract_nav_height' => '',
        'flickity_controls' => '',
        'flickity_overflow' => '',
        ...
        'category_button_text_color' => '#ffffff',
        'enable_sortable' => '',
        ...
        'vertical_list_read_more_style' => 'text',
        'vertical_list_custom_text_color' => '',
        'content_next_to_image_excerpt_pos' => 'default',
        ...
        'content_next_to_image_divider' => 'no',
        'content_next_to_image_divider_color' => '',
        'custom_fields' => '',
  ...
  public static function get_data_attributes( $atts ) {
    ...
    if( 'none' !== $border_radius ) {
      $style_variables[] = '--post-grid-border-radius:'.esc_attr($border_radius).';';
    }

    // Light/Dark text color overrides.
    $allowed_light_styles = array('content_overlaid','content_under_image','content_next_to_image','mouse_follow_image');
    $allowed_dark_styles  = array('content_overlaid','content_under_image','content_next_to_image','mouse_follow_image');

    if( !empty($light_text_color) &&
        in_array($grid_style, $allowed_light_styles, true) &&
        'light' === $text_color ) {
      $style_variables[] = '--post-grid-light-text-color:'.esc_attr($light_text_color).';';
    }
    if( !empty($dark_text_color) &&
        in_array($grid_style, $allowed_dark_styles, true) &&
        'dark' === $text_color ) {
      $style_variables[] = '--post-grid-dark-text-color:'.esc_attr($dark_text_color).';';
    }
    if( !empty($light_text_color_hover) &&
        'content_overlaid' === $grid_style &&
        'light' === $text_color_hover ) {
      $style_variables[] = '--post-grid-light-text-hover-color:'.esc_attr($light_text_color_hover).';';
    }
    if( !empty($dark_text_color_hover) &&
        'content_overlaid' === $grid_style &&
        'dark' === $text_color_hover ) {
      $style_variables[] = '--post-grid-dark-text-hover-color:'.esc_attr($dark_text_color_hover).';';
    }

    $meta_gap_value = trim( $meta_gap );
    if ( '' !== $meta_gap_value && '10' !== $meta_gap_value ) {
      if ( 'px' !== substr( $meta_gap_value, -2 ) ) {
        $meta_gap_value .= 'px';
      }
      $style_variables[] = '--post-grid-meta-gap:'.esc_attr( $meta_gap_value ).';';
    }

    $category_button_text_color_value = trim( $category_button_text_color );
    if ( '' !== $category_button_text_color_value && '#ffffff' !== strtolower( $category_button_text_color_value ) ) {
      $style_variables[] = '--post-grid-category-button-text-color:'.esc_attr( $category_button_text_color_value ).';';
    }

    if ( ! empty( $style_variables ) ) {
      $data_attributes .= 'style="'.implode( '', $style_variables ).'" ';
    }
