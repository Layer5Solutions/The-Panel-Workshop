...
extract(shortcode_atts(array(
  ...
  'delay' => ''), $atts));
	...
	// Calculate height.
	$is_percentage = strpos($custom_height,'%') !== false;
  ...
  if ($line_type === 'Small Thick Line' || $line_type === 'Small Line' ) {
    ...
    $height  = (!empty($custom_height) && !$is_percentage) ? 'style="height: '.esc_attr($calculated_height).';"' : null;
  ...
  // Opacity and percentage height for wrapper (only for No Line type).
  $styles = '';
  if( !empty($divider_opacity) && $divider_opacity != '100' ) {
    $styles .= 'opacity: ' . (floatval($divider_opacity)/100) . ';';
  }
  if( !empty($custom_height) && $is_percentage && ($line_type === 'No Line' || empty($line_type)) ) {
    if( !empty($styles) ) {
      $styles .= ' ';
    }
    $styles .= 'height: ' . esc_attr($calculated_height) . ';';
  }
  if( !empty($styles) ) {
    $styles = ' style="' . $styles . '"';
  }
