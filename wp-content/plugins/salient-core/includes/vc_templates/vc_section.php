...
if ( ! defined( 'ABSPATH' ) ) {
   ...
   extract(shortcode_atts(array(
	...
	'background_image_animation_delay' => '',
  ...
  $nectar_using_VC_front_end_editor = ($nectar_using_VC_front_end_editor == 'true') ? true : false;
		...
		foreach( $bg_img_arr as $viewport => $image ) {
			...
			if( !empty($bg_img_arr[$viewport]['src']) ) {
				...
				if( 'custom' === $background_image_position ) {
					...
					$bg_img_arr[$viewport]['props'] .= 'background-position: '. esc_attr($background_image_position) .'; ';
				}
			...
			if( !empty($background_color) ) {
				...
				$bg_img_arr[$viewport]['props'] .= 'background-color: '. esc_attr($background_color).'; ';
    ...
    if( $video_bg && !empty($video_webm) || $video_bg && !empty($video_mp4) || $video_bg && !empty($video_external) ) {
      if( $background_image_animation === 'zoom-out-reveal' || $background_image_animation === 'slight-zoom-out-reveal' ) {
        $background_image_animation = 'none';
      }
    }
		...
		if( strtolower($parallax_bg) === 'true' && $page_full_screen_rows !== 'on' ){
			...
			if ( !empty($parallax_bg_scroll_effect) && $parallax_bg_scroll_effect !== 'parallax' ) {
				$parallax_class .= ' parallax_effect--'.esc_attr($parallax_bg_scroll_effect);
			}
		} else {
			$parallax_class = '';
			$parallax_speed = null;
      ...
      if( !empty($zindex) ) {
		...
		if ( $color_change_section === 'true' && !empty($background_color) ) {
			...
			$color_change_section_attrs = ' data-color-change-section-bg-color="'.esc_attr($background_color).'" data-color-change-section-text-color="'.esc_attr($color_change_section_text_color).'"';
		}
		...
		if( $page_full_screen_rows === 'on' && $page_full_screen_rows_animation === 'none' && !empty($background_color) ) {
		...
		$row_id = (!empty($el_id) && $page_full_screen_rows !== 'on') ? $el_id: uniqid("fws_");

		$fullscreen_anchor_id = null;
		if( $page_full_screen_rows === 'on' && !empty($el_id) ) {
			$fullscreen_anchor_id = 'data-fullscreen-anchor-id="'.$el_id.'"';
		}
		...
		if( function_exists('nectar_el_dynamic_classnames') ) {
			$dynamic_el_styles = nectar_el_dynamic_classnames('vc_section', $atts);
		} else {
			$dynamic_el_styles = '';
		}
	...
	if( $background_image_animation === 'slight-zoom-out-reveal' ) {
		$background_image_animation = 'zoom-out-reveal';
		$main_class .= 'bg-animation--slight ';
	}

	$css_classes_combined = 'nectar_section wpb_row vc_row '. $top_level_class . $main_class . $disable_class . $equal_height_class . $parallax_class . $vertically_center_class . ' '. $class . $dynamic_el_styles;
	...
	if( $page_full_screen_rows !== 'on' &&
		...
		// sticky row mobile.
		$sticky_row_mobile_class = '';
		if( isset($atts['sticky_row_mobile']) && 'true' === $atts['sticky_row_mobile'] ) {
			$sticky_row_mobile_class = ' nectar-sticky-row-wrap--mobile';
		}
	   ...
	   echo'
		<section id="'. esc_attr($row_id) .'" '.$fullscreen_anchor_id . $border_radius_attrs .' data-column-margin="'.esc_attr($column_margin).'" '.$midnight_attr.' '.$row_percent_padding_attr . $json_animation_attrs . $bg_mobile_hidden. $color_change_section_attrs .' class="'.nectar_clean_classnames($css_classes_combined).'" '.$using_custom_text_color.' style="'.$style.'">';
	...
	if( !empty($background_color) && in_array($background_image_animation, array('fade-in','clip-path')) ) {
		$base_bg_color_style = 'style="background-color: '. esc_attr($background_color).';" ';
	}

	echo '<div class="'.nectar_clean_classnames(implode(' ',$row_bg_classes)).'" data-bg-animation="'.esc_attr($background_image_animation).'" data-bg-animation-delay="'.esc_attr($background_image_animation_delay).'" data-bg-overlay="'.esc_attr($using_bg_overlay).'"><div class="inner-wrap row-bg-layer' . $using_image_class . '" '.$base_bg_color_style.'>';
    ...
    echo '</div>';
		...
		?>
			...
			<div class="nectar-video-wrap row-bg-layer" data-bg-alignment="<?php echo esc_attr( $background_image_position ); ?>">
    ...
    echo '<div class="row_col_wrap_12 span_12 '. esc_attr(strtolower($text_color)) .' '. esc_attr($text_align) . esc_attr($css_perspective_class).'">'. do_shortcode($content) .'</div>';
	...
	if( $page_full_screen_rows !== 'on' ) {
		echo '</section>';
		// Sticky row needs its wrapper closed.
		if( 'true' === $sticky_row ) {
			echo '</div>';
		}
	}
	...
	if( $page_full_screen_rows === 'on' ) {
      echo '</div></div></div></section><!--inner-wrap-->';
	}
